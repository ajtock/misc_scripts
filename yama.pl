...
sub bowtieAlignment {
	...
	if($useBowtie) {
	    ...
	    $bowtie_system_call = "bowtie $bowtie_options -x $genomeMethFile $bowtieinfiles --sam $bowalignFile";	
	} else {
	    if($pairedend) {	
	    my $bowtieinfiles_1 = join ",", @{$fastqfiles_p1_ref};
	    my $bowtieinfiles_2 = join ",", @{$fastqfiles_p2_ref};
	    
	    $bowtie_system_call = "bowtie2 --no-unal $bowtie2_options -x $genomeMethFile -1 $bowtieinfiles_1 -2 $bowtieinfiles_2 -S $bowalignFile";
	    } else {
		my $bowtieinfiles = join ",", @{$fastqfiles_ref};
		$bowtie_system_call = "bowtie2 --no-unal $bowtie2_options -x $genomeMethFile -U $bowtieinfiles -S $bowalignFile";
